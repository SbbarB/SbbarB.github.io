        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('🎉 Digital Closet Loaded!');
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    try {
                        ...
                        console.log('🌤️ Real weather updated based on location:', currentWeather);
                    } catch (apiError) {
                        ...
                        updateWeatherDisplay();
                        console.log('🌤️ Fallback weather used:', currentWeather);
                    }
                },
                (error) => {
        ...
        function useDefaultWeather() {
            ...
            updateWeatherDisplay();
            console.log('🌤️ Using default weather:', currentWeather);
        }
                ...
                img.onload = async () => {
                    ...
                    try {
                        if (model) {
                            ...
                            if (confidence < 0.1) {
                                console.warn('⚠️ Low confidence prediction, using fallback');
                                resolve({
                                    category: 'clothing',
                            ...
                            console.warn('⚠️ Models not loaded, using fallback classification');
            ...
            return new Promise((resolve, reject) => {
                ...
                reader.onload = (e) => {
                    ...
                    img.onload = () => {
                        ...
                        console.log(`📉 Compressed image: ${(e.target.result.length / 1024).toFixed(1)}KB → ${(compressedDataURL.length / 1024).toFixed(1)}KB`);
        ...
        async function addProcessedItems() {
            ...
            try {
                ...
                if (hasDemoItems) {
                    console.log('🗑️ Removing demo items as user is adding real clothing');
        ...
        async function startCamera() {
            try {
                ...
                if (isSafari || isIOS) {
                    console.log('📱 Safari detected - ensuring user gesture initiated camera request');
                }
                ...
                if (isSafari || isIOS) {
                    ...
                    console.log('📹 Using Safari/iOS optimized constraints');
                } else {
                    constraints = {
                        video: {
                ...
                try {
                    ...
                    console.log('✅ getUserMedia succeeded with full constraints');
                } catch (constraintError) {
                    console.warn('📹 Failed with full constraints, trying basic constraints:', constraintError);
                    ...
                    try {
                        ...
                        console.log('✅ getUserMedia succeeded with basic constraints');
                    } catch (basicError) {
                        console.error('❌ Even basic constraints failed:', basicError);
                        throw basicError;
                    }
                }
                    ...
                    const startPlayback = () => {
                        video.play().then(() => {
                            ...
                            if (isSafari || isIOS) {
                                ...
                                console.log('📹 Safari detected - requiring user interaction for video play');
                ...
                if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
                    ...
                    if (isSafari || isIOS) {
                        message = '📱 Camera busy: Close other apps using the camera (like FaceTime, Camera app, or other browser tabs) and try again.';
                    } else {
                    ...
                    if (isSafari || isIOS) {
                        message = `🔧 Safari Camera Error: ${error.message}. Try refreshing the page or restarting Safari.`;
                    } else {
                        message = `Camera error: ${error.message}`;
                    }
                }

                console.error('📹 Detailed camera error:', { name: error.name, message: error.message, constraint: error.constraint });
            ...
            helpDiv.innerHTML = `
                <div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" style="z-index: 9999;">
                    <div class="bg-white rounded-lg p-6 max-w-md mx-4 text-black">
                        <h3 class="text-lg font-bold mb-4">🔒 Safari Camera Access</h3>
                        <div class="space-y-3 text-sm">
                            <p><strong>Step 1:</strong> Look for the camera icon 📷 in Safari's address bar</p>
                            <p><strong>Step 2:</strong> Tap/click it and select "Allow"</p>
                            <p><strong>Step 3:</strong> If no icon appears, tap "aA" → "Website Settings" → Camera → "Allow"</p>
                            <p><strong>iOS Users:</strong> Check Settings → Safari → Camera & Microphone Access</p>
        ...
        function stopCamera() {
            ...
            if (currentCamera) {
                ...
                if (stopBtn) {
                    stopBtn.remove();
                    console.log('🛑 Stop button removed');
                }

                console.log('✅ Camera stopped successfully');
            } else {
                console.log('⚠️ No camera to stop');
            }
        }

        // Legacy getUserMedia fallback for older Safari versions
        function startCameraLegacy() {
            console.log('📹 Using legacy getUserMedia for older browsers');
            ...
            getUserMedia.call(navigator, constraints,
                function(stream) {
                    console.log('✅ Legacy camera stream obtained');
        ...
        async function initializePoseDetection() {
            try {
                console.log('🧠 Loading MediaPipe pose detection...');
            ...
            if (outfitItems.length > 0 && poseLandmarker) {
                try {
                    ...
                    if (poseLandmarkResult.landmarks && poseLandmarkResult.landmarks.length > 0) {
                        ...
                        ctx.font = '12px Inter';
                        ctx.textAlign = 'left';
                        ctx.fillText('✓ Person detected - Enhanced AR active', 15, 25);
                        ...
                        ctx.font = '12px Inter';
                        ctx.textAlign = 'left';
                        ctx.fillText('⚠️ Move into camera view for better fit', 15, 25);
                    }
                } catch (error) {
                ...
                ctx.font = '14px Inter';
                ctx.textAlign = 'left';
                ctx.fillText('🎭 AR Try-On Active', 15, 28);
                ctx.font = '11px Inter';
                ctx.fillText(`${outfitItems.length} items • ${Math.round(opacity * 100)}% opacity • ${Math.round(scale * 100)}% scale`, 15, 45);
        ...
        async function arDetectionFrame() {
            ...
            try {
                ...
                document.getElementById('arClothingActive').textContent = (arProcessedClothing && arProcessedClothing.length > 0 && arClothingVisible) ? '✅' : '❌';
                arFrameCount++;
            } catch (error) {
        ...
        function defineBodySegments(landmarks, width, height) {
            ...
            console.log('🔬 defineBodySegments called, landmarks:', landmarks ? landmarks.length : 'null');

            if (!landmarks || landmarks.length === 0) {
                console.error('❌ No landmarks provided to defineBodySegments');
                return segments;
            }
        ...
        function getBodySegmentsForClothing(landmarks, category, width, height) {
            console.log(`🔍 getBodySegmentsForClothing called for ${category}`);
            ...
            console.log('🔍 All segments created:', Object.keys(allSegments));
            ...
            if (['shirt', 'top', 'blouse', 'jacket', 'coat', 'sweater', 'hoodie'].includes(category)) {
                ...
                console.log('👕 Creating shirt segments');
        ...
        function drawSegmentedClothing(ctx, image, segments, scale, offsetY, widthMult) {
            ...
            segments.forEach((segment, idx) => {
                ...
                if (!segment.boundary) {
                    console.error(`  ❌ Segment ${idx} (${segment.name}) has no boundary`);
                    return;
                }

                console.log(`  ✓ Drawing segment ${idx} (${segment.name}), boundary points: ${segment.boundary.length}`);
                ...
                try {
                    ...
                    console.log(`    ✅ Drew image portion UV(${uv.u0.toFixed(2)},${uv.v0.toFixed(2)}) to (${uv.u1.toFixed(2)},${uv.v1.toFixed(2)})`);
                } catch (error) {
        ...
        function getClothingPositionByCategory(landmarks, category, width, height) {
            console.log('🎯 Positioning item:', category, 'Total landmarks:', landmarks.length);
            ...
            if (!leftShoulder || !rightShoulder) {
                console.warn('❌ Missing shoulder landmarks for:', category);
                return null;
            }
            ...
            const categoryLower = category.toLowerCase();
            console.log('🔍 Category match for:', categoryLower);
            ...
            // Upper body clothing (shirts, jackets, etc.)
            if (['shirt', 'top', 'blouse', 'jacket', 'coat', 'sweater', 'hoodie'].includes(categoryLower)) {
                console.log('👕 Positioning as upper body item');
            ...
            // Lower body clothing (pants, jeans, etc.)
            if (['pants', 'jeans', 'trousers', 'shorts', 'leggings'].includes(categoryLower)) {
                console.log('👖 Positioning as lower body item');
                if (leftHip && rightHip) {
                ...
                console.warn('❌ Missing hip landmarks for pants');
                return null;
            }

            // Full body clothing (dresses, jumpsuits)
            if (['dress', 'skirt', 'jumpsuit', 'romper'].includes(categoryLower)) {
                console.log('👗 Positioning as full body item');
            ...
            // Footwear
            if (['shoes', 'boots', 'sneakers', 'sandals', 'heels'].includes(categoryLower)) {
                console.log('👞 Positioning as footwear');
                if (leftAnkle && rightAnkle) {
                ...
                console.warn('❌ Missing ankle landmarks for shoes');
                return null;
            }

            // Accessories (bags, jewelry, etc.)
            if (['bag', 'purse', 'handbag', 'backpack', 'accessory', 'jewelry', 'watch', 'bracelet'].includes(categoryLower)) {
                console.log('👜 Positioning as accessory');
                ...
                console.warn('❌ Missing wrist/elbow landmarks for accessory');
                return null;
            }

            // Hats and head accessories
            if (['hat', 'cap', 'beanie', 'helmet', 'headband'].includes(categoryLower)) {
                console.log('🎩 Positioning as headwear');
                ...
                console.warn('❌ Missing nose landmark for hat');
                return null;
            }

            // Default case - treat as upper body
            console.log('⚠️ Unknown category, defaulting to upper body positioning:', categoryLower);
            return {
                x: shoulderCenterX,
        ...
        function checkARSupport() {
            ...
            document.getElementById('cameraSupport').innerHTML = `📹 Camera: <span class="${cameraSupported ? 'text-green-400' : 'text-red-400'}">${cameraSupported ? 'Supported' : 'Not Supported'}</span>`;
            const httpsSupported = location.protocol === 'https:' || location.hostname === 'localhost';
            document.getElementById('httpsSupport').innerHTML = `🔒 HTTPS: <span class="${httpsSupported ? 'text-green-400' : 'text-red-400'}">${httpsSupported ? 'Supported' : 'Required'}</span>`;
            const canvas = document.createElement('canvas');
            const webglSupported = !!(canvas.getContext('webgl') || canvas.getContext('experimental-webgl'));
            document.getElementById('webglSupport').innerHTML = `🎮 WebGL: <span class="${webglSupported ? 'text-green-400' : 'text-red-400'}">${webglSupported ? 'Supported' : 'Not Supported'}</span>`;
        }
